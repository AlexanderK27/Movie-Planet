...
router.get('/soon', async (req, res) => {
	...
	try {
		...
		if (movies.length === 0) {
			return res.json({
				message: 'These are all movie premieres we know about'
			})
		}
...
router.get('/:id', async (req, res) => {
	try {
		...
		if (!movie) {
			return res
				.status(404)
				.json({ message: 'No movie found for your request' })
		}
		res.json(movie)
	} catch (e) {
		res.status(500).json({
...
router.get('/poster/:id', async (req, res) => {
	try {
		...
		if (!movie || !movie.description.poster) {
			return res
				.status(404)
				.json({ message: 'Could not find such poster' })
		}
