...
export const SessionPage = () => {
    ...
    const pickSeatHandler = (ticketData) => {
        ...
        const isAlreadyInArr = ticketsArr.find( el => el.row === newTicket[2] && el.seat === newTicket[1])
        ...
        ticketsArr.push({price: newTicket[3], row: newTicket[2], seat: newTicket[1]})
    ...
    const getSession = useCallback( async () => {
        try {
            ...
            const fetched = await request(`/api/movies/session/${sessionId}?mv=${movieId}`, 'GET')
    ...
    const buyTicketHandler = async () => {
        if (tokenValue) {
            ...
            const orderNum = parseInt(Date.now().toString().slice(0, 6) + Date.now().toString().slice(9))
            const orderPOST = []
            const orderPATCH = []
            ...
            try {
                await request('/api/tickets/purchase', 'POST', {
                    order: orderPOST
                }, { Authorization: `Bearer ${tokenValue}` })
            } catch (e) { }
            try {
                await request('/api/tickets/purchase', 'PATCH', {
                    order: orderPATCH
                }, { Authorization: `Bearer ${tokenValue}` })
            } catch (e) { }
    ...
    if (hall) {
        ...
        document.title = `Pick a seat | ${hallName.toString().replace(/,/g, '')} hall`
    }
            ...
            { !loading && (session && hall) && 
                <div className="session-page">
                    ...
                    <div className="movie-data">
                        <h2>{movieName}</h2>
                        <p>{moment(sesDate).format('MMMM D')} {session.time}</p>
                    </div>
                    <div className="purchase-data">
                        <div>
                            ...
                            <div className="neon"></div>
                            {
                                tickets.map( (ticket, index) => {
                                    return (
                                        <div key={ticket.row * Math.random()} className="ticket">
                                            <div>
                                                {index + 1}.
                                                &nbsp;Row: {ticket.row}, 
                                                Seat: {ticket.seat}
                        ...
                        <div>
                            <p>Total: ${total}</p>
                            <button disabled={!!!tokenValue} onClick={openModalHandler}>Buy</button>
                        </div>
                        { !tokenValue ? <p className="warning-message">Sign in to buy a ticket</p> : null}
                    </div>
