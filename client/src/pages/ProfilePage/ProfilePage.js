...
export const ProfilePage = () => {
    ...
    useEffect( () => { getTickets() }, [getTickets, token])
    ...
    useEffect( useCallback( () => {
        if (qrcode) {
            ...
            QRCode.toCanvas(canvas, qrcode, { width: 300, margin: 2 }, (error) => {
                if (error) {
                    canvas.getContext('2d').ctx.fillText('Something went wrong, unable to generate a QRCode :(', 10, 10)
                }
            })
        }
    }, [qrcode]), [qrcode])
    ...
    const downloadOrderPDF = tickets => {
        pdfMake.createPdf(OrderTemplate(tickets).docDefinition).download(`Movie_Planet_${tickets[0].orderNum}`)
    }
