...
export const MoviesList = ({ movies }) => {
	...
	const dayOfMonth = []
	for (let i = 2; i < 7; i++) {
		weakDayName.push(moment().add(i, 'days').format('dddd'))
	}
	for (let i = 0; i < 7; i++) {
	...
	const movieList = movies.map(movie => {
		if (
			movie.startDate <= dayOfMonth[pickedDay.indIn7Days] / 1000 &&
								...
								? movie.schedule[
										...
										return (
											<Link
												...
												onClick={saveToLocalStorage.bind(
													this,
													[
														movie._id,
		...
		<>
			<div className="calendar">
				...
				{calendar.map((day, index) => {
					...
					if (day.indIn7Days === pickedDay.indIn7Days)
					...
					return (
						...
						>
							...
							<p>
								{moment(dayOfMonth[day.indIn7Days]).format(
									'MMMM D'
								)}
							</p>
						</div>
